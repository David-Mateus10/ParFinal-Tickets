...
class Controller
{
    ...
    public function details($request, $response, $args)
    {
        ...
        if ($user->isGestor() && $ticket->gestor_id !== $user->id) {
    ...
    public function changeStatus($request, $response, $args)
    {
        ...
        if (empty($data['estado']) || !in_array($data['estado'], $validStates)) {
            return $this->error($response, 'Estado inválido', 422);
        }
        ...
        if ($updated) {
            ...
            return $this->success($response, ['message' => 'Estado actualizado correctamente']);
        }
    ...
    public function assign($request, $response, $args)
    {
        ...
        if ($updated) {
            ...
            return $this->success($response, ['message' => 'Ticket asignado correctamente']);
        }
